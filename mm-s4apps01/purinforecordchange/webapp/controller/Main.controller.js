...
function (Base, formatter, xlsx, BusyDialog, MessageBox, Spreadsheet) {
    ...
    return Base.extend("mm.purinforecordchange.controller.Main", {
        formatter: formatter,
        ...
        getPostData: function (aExcelHeadSet, aExcelItemSet) {
            ...
            aExcelHeadSet.forEach(function (excelSet) {
                var oHeader = {};
                oHeader.PURCHASINGINFORECORD = excelSet.PurchasingInfoRecord;
        ...
        _callOData: function (bEvent) {
            var aPromise = [];
            var aGroupKey = this.removeDuplicates(this._postData, ["PURCHASINGINFORECORD"]);
            var aGroupItems;

            for (var m = 0; m < aGroupKey.length; m++) {
                const sPurchasingInfoRecord = aGroupKey[m].PURCHASINGINFORECORD;
                aGroupItems = [];
                for (var n = 0; n < this._postData.length; n++) {
                    if (this._postData[n].PURCHASINGINFORECORD === sPurchasingInfoRecord) {
