    ...
    return Base.extend("pp.zroutingupload.controller.Main", {
        ...
        _initialize: function () {
            ...
            oContextBinding.requestObject().then(function (context) {
                ...
                if (!aAllAccessBtns.some(btn => btn.AccessId === "zroutingupload-View")) {
                ...
                this.getModel("local").setProperty("/authorityCheck", {
                    button: {
                        View: aAllAccessBtns.some(btn => btn.AccessId === "zroutingupload-View"),
                        Clear: aAllAccessBtns.some(btn => btn.AccessId === "zroutingupload-Clear"),
                        Check: aAllAccessBtns.some(btn => btn.AccessId === "zroutingupload-Check"),
                        Excute: aAllAccessBtns.some(btn => btn.AccessId === "zroutingupload-Excute"),
                        Export: aAllAccessBtns.some(btn => btn.AccessId === "zroutingupload-Export")
                    },
                    data: {
        ...
        _callOData: function (bEvent) {
            ...
            var aGroupKey = this.removeDuplicates(aExcelSet, ["Product", "Plant", "ProductionRouting"]);
            var aGroupItems;
            for (var m = 0; m < aGroupKey.length; m++) {
                const sProduct = aGroupKey[m].Product;
                const sPlant = aGroupKey[m].Plant;
                const sProductionRouting = aGroupKey[m].ProductionRouting;
                aGroupItems = [];
                for (var n = 0; n < aExcelSet.length; n++) {
                    if (aExcelSet[n].Product === sProduct && aExcelSet[n].Plant === sPlant && aExcelSet[n].ProductionRouting === sProductionRouting) {
